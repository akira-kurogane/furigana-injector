...
//
//  $Id: feature_index.cpp 159 2008-01-22 15:26:28Z taku-ku $;
//
...
namespace MeCab {
  ...
  bool EncoderFeatureIndex::convert(const char* txtfile, const char *binfile) {
    std::ifstream ifs(txtfile);

    CHECK_FALSE(ifs) << "no such file or directory: " << txtfile;
    ...
    CHECK_FALSE(ofs) << "permission denied: " << binfile;
    ...
    for (std::map<std::string, double>::const_iterator
      ...
      ofs.write(reinterpret_cast<const char*>(&it->second), sizeof(alpha_[0]));
    }

    Darts::DoubleArray da;
    CHECK_FALSE(da.build(key.size(), &key[0], 0, 0, 0) == 0)
